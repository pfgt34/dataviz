...
def appeler_api(endpoint: str, params: dict = None):
    """
    Appelle l'API et retourne les données (cache 5 min)
...
with tab1:
    ...
    fig_produits = px.bar(
        df_produits, x=critere_tri, y='produit', color='categorie', orientation='h',
        title=f"Top {nb_produits} Produits par {labels_criteres[critere_tri]}",
        labels={'ca': 'CA (€)', 'profit': 'Profit (€)', 'quantite': 'Quantité'},
    ...
    with st.expander("📋 Voir le tableau détaillé"):
        st.dataframe(
            df_produits[['produit', 'categorie', 'ca', 'profit', 'quantite']].rename(columns={
                ...
                'ca': 'CA (€)', 'profit': 'Profit (€)', 'quantite': 'Quantité'
            }),
            use_container_width=True, hide_index=True
        )

# ----------------------------------------------------------------------------
# TAB 2 : ANALYSE CATÉGORIES
# ----------------------------------------------------------------------------
with tab2:
    ...
    with col_left:
        fig_cat = go.Figure()
        fig_cat.add_trace(go.Bar(name='CA', x=df_cat['categorie'], y=df_cat['ca'],
                                 marker_color='#667eea', text=df_cat['ca'].apply(lambda x: f"{x:,.0f}€")))
        fig_cat.add_trace(go.Bar(name='Profit', x=df_cat['categorie'], y=df_cat['profit'],
                                 marker_color='#764ba2', text=df_cat['profit'].apply(lambda x: f"{x:,.0f}€")))
    ...
    with col_right:
        fig_marge = px.bar(df_cat, x='categorie', y='marge_pct', title="Marge par Catégorie (%)",
                          color='marge_pct', color_continuous_scale='Viridis',
                          text='marge_pct', height=400)
    ...
    st.dataframe(
        df_cat[['categorie', 'ca', 'profit', 'marge_pct', 'nb_commandes']].rename(columns={
            'categorie': 'Catégorie', 'ca': 'CA (€)', 'profit': 'Profit (€)',
            'marge_pct': 'Marge (%)', 'nb_commandes': 'Nb Commandes'
        }),
        use_container_width=True, hide_index=True
    )
...
with tab3:
    ...
    granularite = st.radio(
        "Période d'analyse", options=['jour', 'mois', 'annee'],
    ...
    fig_temporal.add_trace(go.Scatter(x=df_temporal['periode'], y=df_temporal['ca'],
                                     mode='lines+markers', name='CA',
                                     line=dict(color='#667eea', width=3), fill='tozeroy'), row=1, col=1)
    fig_temporal.add_trace(go.Scatter(x=df_temporal['periode'], y=df_temporal['profit'],
                                     mode='lines+markers', name='Profit',
                                     line=dict(color='#764ba2', width=3)), row=1, col=1)
    fig_temporal.add_trace(go.Bar(x=df_temporal['periode'], y=df_temporal['nb_commandes'],
                                 name='Commandes', marker_color='#f39c12'), row=2, col=1)
...
with tab4:
    ...
    with col_geo1:
        fig_geo_ca = px.bar(df_geo, x='region', y='ca', title="Chiffre d'affaires par Région",
                           color='ca', color_continuous_scale='Blues', text='ca', height=400)
        fig_geo_ca.update_traces(texttemplate='%{text:,.0f}€', textposition='outside')
        st.plotly_chart(fig_geo_ca, use_container_width=True)
    
    with col_geo2:
        fig_geo_clients = px.pie(df_geo, values='nb_clients', names='region',
                                title="Répartition des Clients par Région",
                                color_discrete_sequence=px.colors.qualitative.Set3, height=400)
    ...
    st.dataframe(
        df_geo[['region', 'ca', 'profit', 'nb_clients', 'nb_commandes']].rename(columns={
            'region': 'Région', 'ca': 'CA (€)', 'profit': 'Profit (€)',
            'nb_clients': 'Nb Clients', 'nb_commandes': 'Nb Commandes'
        }),
        use_container_width=True, hide_index=True
    )
...
with col_client1:
    ...
    fig_clients = px.bar(
        ...
        title="Top Clients par CA", labels={'ca_total': 'CA Total (€)', 'nom': 'Client'},
        color='nb_commandes', color_continuous_scale='Viridis', height=400
    )
